...
% Raman shift range
[~,i1] = min(abs(str2num(XkaiserF.v)-515));  
[~,i2] = min(abs(str2num(XkaiserF.v)-1870)); 
...
% Raman shift range
[~,i1] = min(abs(str2num(XmmF.v)-515));  
[~,i2] = min(abs(str2num(XmmF.v)-1870)); 
...
% -------------------------------------------------------------------------
% MM ----------------------------------------------------------------------
Xmm = saisir_derivative(Xmm,polorder,wd,derorder);
nvar = length(Xmm.v);
Xmm = selectcol(Xmm,5:(nvar-4)); % Remove edge effects (the edge points on each side)

% Kaiser ------------------------------------------------------------------
Xkaiser= saisir_derivative(Xkaiser,polorder,wd,derorder);
nvar = length(Xkaiser.v);
Xkaiser = selectcol(Xkaiser,5:(nvar-4)); % Remove edge effects (the edge points on each side)
...
% MM ----------------------------------------------------------------------
XmmF = saisir_derivative(XmmF,polorder,wd,derorder);
nvar = length(XmmF.v);
XmmF = selectcol(XmmF,5:(nvar-4)); % Remove edge effects (the edge points on each side)

% Kaiser ------------------------------------------------------------------
XkaiserF= saisir_derivative(XkaiserF,polorder,wd,derorder);
nvar = length(XkaiserF.v);
XkaiserF = selectcol(XkaiserF,5:(nvar-4)); % Remove edge effects (the edge points on each side)
...
% -------------------------------------------------------------------------
% Delete surplus replicates
XkaiserF = delete_from_identifier(XkaiserF,10,'c');

% Merge spectral data from all three meat types ---------------------------
...
grouppos = 3:6;
idpos = 8:9;
saisir_explore_reference_data(References,grouppos, idpos) 
...
% -------------------------------------------------------------------------
ref_idpos = 1:9; % string position in reference id tags for matching
x_idpos = 1:9; % string position in spectral/data blocks for matching
reference_names = References.i(:,ref_idpos);
nref = size(References.d,2); % Number of reference analytes
Y_subsets = {};
...
p = 0.001; % Asym weighting parameter
...
%% EXPLORE PREPROCESSED NIR SPECTRA
% -------------------------------------------------------------------------
isubset = 21;
plot_NIR(str2num(X_prep_snv_subsets{isubset}.v), X_prep_snv_subsets{isubset}.d); ylim([-2 2]); 
grid on
...
% -------------------------------------------------------------------------
isubset = 21;
plot_NIR(str2num(X_prep_snv_fatband_subsets{isubset}.v), X_prep_snv_fatband_subsets{isubset}.d); ylim([-2 2]); 
grid on
fatPercent = Y_subsets{isubset}.d(:,1);
color_by(fatPercent);
cb = colorbar;
...
%% ESTABLISH QUICK FAT MODEL FOR TEST ON SUPPLEMENTARY MEAT STORAGE TIME DATA
% -------------------------------------------------------------------------
...
plot_regcoef(X.v, B',  0.6,'findPeaks', 'yes');
figure; plot(Y_type.d(:,1),YpredCal.d,'O')
ylabel('Predicted')
xlabel('Target')
box off; grid on
title('Cal')
...
% Remember to save model if update in DataAnalysis_MeatStorageTime script 
% is wanted.
...
%% ALL COMBINATIONS VALIDATION SCHEME / ACROSS GRINDING DEGREES
% -------------------------------------------------------------------------
...
[FIG, Regcoeff, Performance, Residuals] = PLSR_allcombos_validation_from_identifier(X, Y, target, idpos, aopt, reps_idpos, 'aoptAlg','STATIC' , 'plotLimits', [-5 50]);

% Update pred vs target plot with proper naming convention 
leg = get(gca, 'Legend');
...
% Plot the regression coefficients with proper naming convention
...
% Plot performance summary with proper naming convention
...
[FIG, Regcoeff, Performance, Residuals] = PLSR_allcombos_validation_from_identifier(X, Y, target, ...
                                            idpos, aopt,reps_idpos, 'aoptAlg','STATIC', 'plotLimits', [-5 50]); 
...
xline(1211, 'Color', [0.5 0.85 0.5],'Linewidth', 50)
legend(cellstr(Regcoeff.i),'Location', 'northwest')
