function vect=find_peaks_prom_saisir(saisir,row,threshold,gap,min_max)
...
%
% Suggested default values for baselien corr spectra: vect = find_peaks(X,20,200,10,0)
      ...
      if(abs(x(j))>threshold)
         ...
         if (b(1)==aux)
            ...
            line([posx posx],[0 x(j)]);
         	text(posx,x(j),num2str(round(str2num(saisir.v(j,:)),1)),'FontSize',8);   
         end   
      end   
    end   
 end
