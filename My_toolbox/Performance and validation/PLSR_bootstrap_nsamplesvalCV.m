function [Result_summary,MetricsPerDraw] = PLSR_bootstrap_nsamplesvalCV(X, Y, target, ndraws, nsamples, idpos,reps_idpos, ncomp, varargin)
% ------- Bootstrap validation as a function of number of samples ---------
% A bootstrap validation version for PLSR validation, with separate cal and
% test sets, where model complexity choice is based on LooCV performance in
% the calibration set. The validation is done for different variations in 
% number of calibration samples, ensuring a balanced design in each sample 
% selection (based on three groupings of the target value).
% -------------------------------------------------------------------------
%
%   INPUT:
...
%               reps_idpos- Id position (indices) in strings used to define
%                           which spectra are from same samples (replicates)
...
%   to 'STATIC' 
% * aoptAlg : 'STATIC' / 'WM' (default) / 'UNSC'
%
% -------------------------------------------------------------------------
% REFERENCES
% -------------------------------------------------------------------------
...
p = inputParser;
   ...
   addRequired(p,'reps_idpos'); % positional arg
   ...
   parse(p,X, Y, target, ndraws, nsamples, idpos, reps_idpos, ncomp, varargin{:});
...
for i = 1:length(nsamples)
    ...
    for j = 1:ndraws  
        ...
        for k = 1:length(Xcal_segments)
            ...
            randind = randi(nsubsamp,ceil(nsamp/3),1); % generate random sample indices within the correct index range 
            Xcal_rand_subsets{k} = selectrow(Xcal_segments{k},randind);
            Ycal_rand_subsets{k} = selectrow(Ycal_segments{k},randind);
        end

        % Merge the three random selection ref. value range groups
        Xcal_randsel = saisir_rowmerge(Xcal_rand_subsets{:});
        Ycal_randsel= saisir_rowmerge(Ycal_rand_subsets{:});

        % Calibration -----------------------------------------------------

        % Cross validation on calibration set, find optimal num. of comps.  
        [Predictions,Regcoeff, Performance] = PLSR_CV_from_identifier(Xcal_randsel, Ycal_randsel, target,reps_idpos, ncomp,'showPlot','no','aoptAlgMetric','RMSECV', 'aoptAlg',p.Results.aoptAlg);
        aopt = Performance.lv;

        % Make PLSR calibration model with suggested number of comps from CV
        [b0,B,T,W,P,q] = pls_nipals(Xcal_randsel.d,Ycal_randsel.d(:,target),aopt);         
        
        % Validation ------------------------------------------------------

        % Predict test set with the all number of components from CV on
        % cal set:
...
% Plot mean RMSEP_corr as a function of number of samples
figure
hold on
plot(nsamples, squeeze(Result_summary.DescrStatsMetrics.d(2,8,:)),'-o')
plot(nsamples, squeeze(Result_summary.DescrStatsMetrics.d(6,8,:)),'--')
plot(nsamples, squeeze(Result_summary.DescrStatsMetrics.d(7,8,:)),'--')
xlabel('Number of samples', 'FontSize',18)
