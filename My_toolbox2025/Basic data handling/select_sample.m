function Dataset = select_sample(Dataset, selection)
% Select samples defined by selection from a saisir Dataset. The selection can be
...
if isa(selection,'double')
    Dataset.d = Dataset.spec(selection,:);
    Dataset.i = Dataset.i(selection,:);
    Dataset.v = Dataset.v;
