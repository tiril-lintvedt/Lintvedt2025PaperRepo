function [Xcorr, Baseline] = saisir_modpoly(X, varargin)
% [Xcorr, baseline] = modpoly(X, options)
%
% The function modpoly fits and subtracts a polynomial baseline from the
% spectra X, using the saisir data structure. A separate baseline is fitted 
% to each spectrum, and returned in separate saisir structure.
%
% Inputs:
...
[nSamp,nVars] = size(X.d);
meanVal = mean(mean(X.d));
...
p.parse(X.d, varargin{:});
opts = p.Results;

% Initialise the fitted baseline
baseline = zeros(size(X.d));
baseline(:,opts.interval) = X.d(:,opts.interval);
...
% Convert baseline to saisir structure:
Baseline.d = baseline;
Baseline.v = X.v;
Baseline.i = X.i;

% Subtract the fitted baseline from the spectra, and convert to saisir:
Xcorr.v = X.v;
Xcorr.i = X.i;
Xcorr.d = X.d - baseline;
